...
const EditAboutUsPage = () => {
    ...
    const fetchSection = useCallback(async () => {
        try {
            const response = await api.get(`/api/MyHome2U/AboutUs/SingleSection/${id}`);
            const section = response.data.section;
            setSectionTitle(section.sectionTitle);
            setSectionContent(section.sectionContent);
            setImagePreview(section.sectionImage.url);
        } catch (error) {
            console.error(error);
        }
    }, [id]);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await api.put(`/api/MyHome2U/AboutUs/UpdateSection/${id}`,formData);
        ...
        <div className="flex min-h-screen bg-black">
            ...
            <main className="flex-1 p-8 bg-white">
                <div className="overflow-x-auto">
                    <h1 className="text-3xl font-semibold text-gray-800 mb-6">Update Section</h1>
